...
<div id="content" class="content">
    ...
    <h1 class="page-header">Form Reture<small></small></h1>
    <div class="row">
        <div class="col-xl-12">
            <!-- begin panel -->
                ...
                <div class="panel-body">
                        ...
                        <div class="row">
                            <div class="col-md-3" style="height: 400px;">
                                <form id="mydata" action="{{url('/retur')}}" method="post" enctype="multipart/form-data">
                                    @csrf
						            <input type="hidden" name="id" value="{{$id}}">
                                    @if($id>0)
                                    <div class="form-group">
                                        <label>NOMOR RETURE</label>
                                        <input type="text" disabled value="{{$data->no_retur}}" class="form-control" >
                                    </div>
                                    @endif
                                    ...
                                    <div class="form-group">
                                        @if($data->sts>1)
                                            ...
                                            <div class="btn-group mr-2 sw-btn-group" role="group">
                                                <span class="btn btn-secondary sw-btn-prev " onclick="location.assign(`{{url('retur')}}`)"><i class="fas fa-arrow-alt-circle-right fa-rotate-180"></i> Kembali</span>
                                            </div>
                                            @else
                                            <div class="btn-group mr-2 sw-btn-group" role="group">
                                                <span class="btn btn-secondary sw-btn-prev " onclick="location.assign(`{{url('retur')}}`)"><i class="fas fa-arrow-alt-circle-right fa-rotate-180"></i> Kembali</span>
                                    ...
                                    <table class="table table-bordered m-b-0">
                                        ...
                                        <tbody>
                                            <?php
                                                $total=0;
                                            ?>
                                            @foreach(get_transaksi($data->no_retur) as $nk=>$trs)
                                            <?php
                                                $total+=$trs->total;
                                            ?>
                                            <tr>
                                                <td>{{$nk+1}}</td>
                                                <td><span class="btn btn-xs btn-red" onclick="hapus_barang(`{{$data->no_retur}}`,{{$trs->id}})"><i class="fas fa-times-circle"></i></span></td>
                                                <td>{{$trs->kode}}</td>
                            ...
                            <div class="col-md-7" style="display:flex;background: #f2f2ff; border: double #c9b2b2; padding:1%;">
                                ...
                                @else
                                    <div class="btn-group">
                                        ...
                                        <button class="btn btn-white active" onclick="proses(`{{$data->no_retur}}`)"><i class="fas fa-check-square"></i>  Selesai</button>
                                    </div>
                                @endif
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form class="form-horizontal form-bordered" id="mydatatambah" action="{{url('/produk')}}" method="post" enctype="multipart/form-data">
                            @csrf
                                <input type="hidden" name="no_transaksi" value="{{$data->no_retur}}">
...
@push('ajax')
    <script>
		...
		function hapus_barang(no_retur,id){
            $.ajax({
                type: 'GET',
                url: "{{url('/retur/hapus_barang')}}",
                data: "no_retur="+no_retur+"&id="+id,
                beforeSend: function(){
		...
		function proses(no_retur){
            $.ajax({
                type: 'GET',
                url: "{{url('/retur/proses')}}",
                    ...
                    success: function(msg){
                        ...
                        if(bat[1]=='ok'){
                            @if($id>0)
                                location.reload();
                            @else
                                location.assign("{{url('retur/form')}}?id="+bat[2])
                            @endif
